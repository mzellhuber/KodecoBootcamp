...
class RecipesAndFavoritesViewUITests: XCTestCase {
  ...
  func testAddFavoritesAndCheckDisplay() throws {
    ...
    let firstRecipeRow = app.collectionViews.buttons.containing(
      NSPredicate(format: "identifier BEGINSWITH 'RecipeRow-'")).firstMatch
    ...
    let predicate = NSPredicate(format: "identifier BEGINSWITH 'RecipeRow-'")
    let secondRecipeRow = app.collectionViews
      .buttons
