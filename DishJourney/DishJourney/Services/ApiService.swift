...
class ApiService {
  ...
  func fetchRecipes(query: String, startIndex: Int, endIndex: Int) async throws -> [Recipe] {
    let queryEncoded = query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
    let urlString = "\(baseUrl)?q=\(queryEncoded)&app_id=\(appId)&app_key=\(appKey)&from=\(startIndex)&to=\(endIndex)"
  ...
  */
  ...
  func createMealPlan(request: MealPlanRequest) async throws -> [Recipe] {
    let calorieRange = "\(request.calorieMin)-\(request.calorieMax)"
    ...
    var queryItems = [
      ...
      URLQueryItem(name: "to", value: String(request.size)),
      URLQueryItem(name: "calories", value: calorieRange),
      URLQueryItem(name: "mealType", value: request.mealType)
    ]

    for dietLabel in request.dietLabels {
      queryItems.append(URLQueryItem(name: "diet", value: dietLabel))
    }

    for healthLabel in request.healthLabels {
